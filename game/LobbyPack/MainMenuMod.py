...
from AuthInfoMod import AuthInfo
from AppPack.SubstateMod import Substate

"""Lobby MainMenu Substate"""
class MainMenu(Substate):
	...
	def __init__(self, screen, fpsclock, sound):
		super(MainMenu, self).__init__(screen, fpsclock, sound)

	"""
		Initialize Menu elements and Authentification info
	"""

	def _initObjects(self):

		#Create Menu List and Structure
		self._list = ['SinglePlayer','MultiPlayer','HighScore','Options','Back']
		...
		self._errorfont = pygame.font.Font(OtherConfigs.FONT_PATH,OtherConfigs.USER_FONT_SIZE*2)
	...
	def _initImages(self):
		...
		self._errorMessageDisplay = self._errorfont.render(self.__errorMessage, True, OtherConfigs.COLOR_RED)	
	...
	def _ticked(self):
		...
		self.user1 = self.authinfo.user_one.username
		self.user2 = self.authinfo.user_two.username

		if self.user1 != "":
			self._usertext1 = self._userfont.render(self.user1+" is logged in.", True, OtherConfigs.COLOR_GREEN)
		else:
			self._usertext1 = self._userfont.render("", True, OtherConfigs.COLOR_RED)
		if self.user2 != "":
			self._usertext2 = self._userfont.render(self.user2+" is logged in.", True, OtherConfigs.COLOR_GREEN)
		else:
			self._usertext2 = self._userfont.render("", True, OtherConfigs.COLOR_RED)
				...
				if event.key == K_RETURN:
					if self.position_selected==0:
						...
						if self.user1 != "" and self.user2 != "":
							self.__setErrorMessage("Error: Only one player must be logged in.")
							self.move(4)
						else:
					...
					if self.position_selected==1:
						...
						if self.user1 != "" and self.user2 != "":
							self._substate = SubstateConfigs.SUBSTATE_INMULTALT
						else:
							self.__setErrorMessage("Error: Only one player is logged in.")
							self.move(3)
					...
					if self.position_selected==3:
						...
						self._substate = SubstateConfigs.SUBSTATE_OPTIONSMENU  
						break
					if self.position_selected==4:
	...
	def _render(self):
		...
		self._screen.blit(self._usertext1, (5, 5))
		self._screen.blit(self._usertext2, (5, 20))
